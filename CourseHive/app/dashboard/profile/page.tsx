    ...
    <motion.div initial="hidden" animate="show" variants={stagger} className="space-y-6 max-w-6xl mx-auto pb-10">
      ...
      <motion.div variants={fade} className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold bg-card border border-border hover:bg-secondary transition-colors text-foreground">
      ...
      <motion.div variants={stagger} className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {statCards.map(({ icon: Icon, label, value }, i) => (
          <motion.div
            key={label}
            variants={fade}
            className={`rounded-2xl p-5 border border-border/ shadow-sm ${i === 0 ? 'text-white' : 'bg-card/50 backdrop-blur-sm hover:shadow-md transition-shadow'}`}
            style={i === 0 ? { background: `linear-gradient(135deg, ${PRIMARY} 0%, var(--primary) 100%)` } : {}}
          >
            {i !== 0 && (
              <button className="float-right w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center hover:bg-slate-100 transition-colors text-slate-400 hover:text-foreground">
                <ArrowUpRight className="w-4 h-4" />
              </button>
            )}
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${i === 0 ? 'bg-card/50 text-white' : 'bg-slate-100 text-foreground'}`}>
              <Icon className="w-5 h-5" />
            </div>
            <p className={`text-sm font-semibold ${i === 0 ? 'text-white/80' : 'text-slate-500'}`}>{label}</p>
            <p className={`text-4xl font-black mt-1 tracking-tight ${i === 0 ? 'text-white' : 'text-foreground'}`}>{value}</p>
          </motion.div>
        ))}
      </motion.div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={fade} className="lg:col-span-1 space-y-6">
          <div className="bg-card/50 backdrop-blur-sm border border-border/ rounded-[28px] overflow-hidden shadow-sm flex flex-col relative group hover:shadow-md transition-shadow">
            {/* Cover Strip */}
            <div className="h-32 relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${PRIMARY} 0%, var(--primary) 100%)` }}>
            ...
            <div className="px-6 pb-6 pt-0 relative flex flex-col items-center">
              {/* Avatar */}
              <div className="w-24 h-24 rounded-full border-4 border-border flex items-center justify-center text-3xl font-black text-white shadow-xl -mt-12 bg-gradient-to-br from-[#f97316] to-[#ea6c0a] relative z-10 group-hover:scale-105 transition-transform duration-300">
                {userName.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
                <div className="absolute bottom-0 right-0 w-6 h-6 bg-emerald-50 dark:bg--900/200 border-2 border-border rounded-full" title="Online" />
              </div>
              ...
              <div className="w-full mt-6 space-y-3">
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 text-sm">
                  ...
                  <span className="font-semibold text-foreground">Joined {new Date(userJoinDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</span>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 text-sm">
                  <MapPin className="w-4 h-4 text-slate-400" />
                  <span className="font-semibold text-foreground">Learning Online</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <motion.div variants={fade} className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card/50 backdrop-blur-sm border border-border/ rounded-[28px] p-8 shadow-sm">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <div className="px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-semibold text-foreground">
                  {userField}
                </div>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="px-4 py-3.5 bg-indigo-50 dark:bg--900/20 border border-indigo-100 rounded-xl text-sm font-bold text-indigo-700 flex items-center gap-2">
          ...
          <div className="bg-card/50 backdrop-blur-sm border border-border/ rounded-[28px] p-8 shadow-sm">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <button className="flex items-center justify-between p-4 bg-card border border-slate-200 hover:border-slate-300 rounded-xl text-sm font-semibold text-foreground transition-all group">
                <span className="flex items-center gap-2"><Shield className="w-4 h-4 text-slate-400 group-hover:text-foreground" /> Change Password</span>
                <ArrowUpRight className="w-4 h-4 text-slate-300 group-hover:text-foreground" />
              </button>
              
              <button className="flex items-center justify-between p-4 bg-card border border-slate-200 hover:border-slate-300 rounded-xl text-sm font-semibold text-foreground transition-all group">
                <span className="flex items-center gap-2"><User className="w-4 h-4 text-slate-400 group-hover:text-foreground" /> Privacy Preferences</span>
                <ArrowUpRight className="w-4 h-4 text-slate-300 group-hover:text-foreground" />
              </button>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-slate-100">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-bold text-sm text-foreground">Danger Zone</h4>
