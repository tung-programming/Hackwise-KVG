...
export const leaderboardService = {
  ...
  getGlobalLeaderboard: async (page: number, limit: number) => {
    ...
    // Always sort by XP directly to avoid stale rank values.
  ...
  getUserRank: async (userId: string) => {
    ...
    const effectiveXp = entry?.total_xp ?? user?.xp ?? 0;
    const effectiveUpdatedAt = entry?.updated_at || now;

    const { count: higherXpCount } = await supabase
      .from("leaderboard")
      .select("*", { count: "exact", head: true })
      .gt("total_xp", effectiveXp);

    const { count: sameXpEarlierCount } = await supabase
      .from("leaderboard")
      .select("*", { count: "exact", head: true })
      .eq("total_xp", effectiveXp)
      .lt("updated_at", effectiveUpdatedAt);

    const computedRank = (higherXpCount || 0) + (sameXpEarlierCount || 0) + 1;
