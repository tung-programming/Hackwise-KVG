...
//    - null/undefined
function normalizeHistoryEntries(rawData) {
    if (!rawData)
        return [];
...
function extractHistorySignals(rawData) {
...
exports.interestsService = {
    getUserInterests: async (userId) => {
        const { data: interests, error } = await database_1.supabase
            .from("interests")
            .select("*")
            .eq("user_id", userId)
    ...
    getInterest: async (userId, interestId) => {
        const { data: interest, error } = await database_1.supabase
            .from("interests")
            .select("*")
            .eq("id", interestId)
            .eq("user_id", userId)
        ...
        const { data: courses } = await database_1.supabase
            .from("courses")
            .select("*")
    ...
    acceptInterest: async (userId, interestId) => {
        const { data: activeInterests } = await database_1.supabase
            .from("interests")
            .select("id, name")
            .eq("user_id", userId)
            .eq("status", "accepted")
        ...
        if (activeInterests && activeInterests.length > 0) {
            throw new errors_1.BadRequestError(`You already have an active interest: "${activeInterests[0].name}". Complete it first or reject it to start a new one.`);
        }
        const { data: interest, error } = await database_1.supabase
            .from("interests")
            ...
            .single();
        if (error || !interest) {
            throw new errors_1.NotFoundError("Interest not found or already processed");
        }
    ...
    rejectInterest: async (userId, interestId) => {
        const { data: interest, error } = await database_1.supabase
            .from("interests")
    ...
    getProgress: async (userId, interestId) => {
        const { data: interest, error } = await database_1.supabase
            .from("interests")
    ...
    canAddNewInterests: async (userId) => {
        const { data: activeInterests } = await database_1.supabase
            .from("interests")
            .select("id")
            .eq("user_id", userId)
...
async function generateRoadmapAsync(userId, interest) {
    try {
        ...
        if (interest.history_id) {
            const { data: historyRecord } = await database_1.supabase
...
- Course names must be specific (e.g. "React useReducer & Context API", not "React Advanced")`;
        let roadmap;
        try {
            roadmap = await gemini_1.default.generateJSON(prompt, 1200);
        }
        catch (parseError) {
            console.error("JSON parse error in roadmap generation, using fallback:", parseError);
            const fallbackResponse = await gemini_1.default.generateWithFlash(`For a ${field}/${type} student learning "${interest.name}", list 5 specific courses as JSON array only: [{"name":"","description":"","resource_url":"https://youtube.com/results?search_query=...","duration":"2h","difficulty":"beginner"}]`, 800);
        ...
        if (roadmap.courses && Array.isArray(roadmap.courses)) {
            ...
            for (let i = 0; i < Math.min(validCourses.length, 7); i++) {
                ...
                if (!resourceUrl || resourceUrl === "null" || !resourceUrl.startsWith("http")) {
                    resourceUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(course.name + " tutorial")}`;
                }
                await database_1.supabase.from("courses").insert({
                    interest_id: interest.id,
                    user_id: userId,
                    name: course.name,
        ...
        if (roadmap.projects && Array.isArray(roadmap.projects)) {
            for (const project of roadmap.projects.slice(0, 3)) {
                if (!project.name)
                    continue;
                await database_1.supabase.from("projects").insert({
                    interest_id: interest.id,
                    user_id: userId,
                    name: project.name,
        ...
        console.log(`✅ Roadmap generated for "${interest.name}": ${roadmap.courses?.length || 0} courses, ${roadmap.projects?.length || 0} projects`);
    }
    catch (error) {
        ...
        for (let i = 0; i < fallbackCourses.length; i++) {
            ...
            await database_1.supabase.from("courses").insert({
                ...
                resource_url: `https://www.youtube.com/results?search_query=${encodeURIComponent(course.name + " tutorial")}`,
                node_order: i + 1,
                is_locked: i !== 0,
                is_completed: false,
